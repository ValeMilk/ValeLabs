...
export default function ProgressMetricCard({
  ...
  const sz = SIZES[size];
  const shell = `relative flex ${sz.minH} w-full flex-col overflow-hidden rounded-[28px] border border-gray-200 bg-white shadow-[0_2px_10px_rgba(0,0,0,0.04)] ${className}`;
    ...
    return (
      <div className={shell} aria-busy="true">
        <div className={`flex flex-1 flex-col ${sz.pad}`}>
          ...
          <div className="mt-6 h-14 w-48 animate-pulse rounded-lg bg-gray-200" />
          <div className="mt-auto h-24 w-full animate-pulse rounded-lg bg-gray-100" />
        </div>
        <div className={`border-t border-gray-200 ${sz.footer}`}>
  ...
  if (!hasData) {
    return (
      <div className={shell}>
        <div className={`flex flex-1 flex-col ${sz.pad}`}>
          <h3 className={`${sz.title} font-semibold tracking-tight text-gray-900`}>{title}</h3>
          <div className="flex flex-1 flex-col items-center justify-center gap-1 py-10 text-center">
            <p className="text-sm font-medium text-gray-900">Sem dados</p>
            <p className="text-xs text-gray-500">Métricas aparecerão quando dados estiverem disponíveis</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className={shell}>
      {/* Zona do gráfico (à direita, atrás do conteúdo) */}
      ...
      <div className={`pointer-events-none relative z-10 flex flex-1 flex-col ${sz.pad}`}>
        ...
        {/* Legenda (apenas em multi-séries) */}
        {isMulti && (
        ...
        <div
          className={`mt-5 ${sz.headline} font-medium leading-none tracking-tight text-gray-900`}
        >
          {displayTotal}
        </div>
      </div>

      {/* Footer opaco: delta à esquerda, stats secundárias à direita */}
      <div
        className={`relative z-10 flex items-center justify-between gap-4 border-t border-gray-200 bg-white ${sz.footer} text-[14px]`}
      >
        <div>
        ...
        {showStats && (
          <div className="flex items-center gap-2.5 text-[12px] text-gray-600">
            <span>
              <span className="font-medium text-gray-900">{fmtCompact(stats.peak)}</span> máx
            </span>
            <span className="opacity-40">·</span>
            <span>
              <span className="font-medium text-gray-900">{fmtCompact(stats.low)}</span> mín
            </span>
            <span className="opacity-40">·</span>
            <span>
